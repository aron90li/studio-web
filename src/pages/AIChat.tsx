...
import { Layout, Button, Avatar, Tag, Typography, Spin, Space } from '@arco-design/web-react'
...
export default function AIChat() {
  ...
  const loadSessions = useCallback(async () => { try { const r = await getSessions(); if (r.data.success) setSessions(r.data.data) } catch { /* */ } }, [])
  const loadMsgs = useCallback(async (id: string) => {
    setLoading(true)
    try { const r = await getSessionMessages(id); if (r.data.success) { setMessages(r.data.data.filter((m: any) => m.role === 'user' || m.role === 'assistant').map((m: any) => ({ id: crypto.randomUUID(), role: m.role, content: m.content, timestamp: new Date(m.createTime).getTime() }))); scroll() } } catch { /* */ }
    setLoading(false)
  }, [scroll])

  const select = useCallback((id: string) => { if (sid === id) return; setSid(id); setMessages([]); loadMsgs(id); focus() }, [sid, loadMsgs])
  const newChat = useCallback(() => { setSid(undefined); setMessages([]); setInput(''); focus() }, [])
  const del = useCallback(async (id: string, e: React.MouseEvent) => { e.stopPropagation(); try { await clearSession(id); await loadSessions(); if (sid === id) newChat() } catch { /* */ } }, [sid, loadSessions, newChat])
  const stop = useCallback(() => { abortRef.current?.abort(); abortRef.current = null; setSending(false); setMessages(p => p.map(m => m.isStreaming ? { ...m, isStreaming: false } : m)) }, [])

  const send = useCallback(() => {
    const text = input.trim()
    if (!text || sending) return
    setInput('')
    const s: string = sid || crypto.randomUUID()
    if (!sid) { setSid(s); setSessions(p => [{ sessionId: s, title: text, messageCount: 0, lastActiveTime: new Date().toISOString() }, ...p]) }
    setSending(true)
    ...
    streamId.current = am.id; setMessages(p => [...p, um, am]); scroll()
    const abort = new AbortController(); abortRef.current = abort
    let content = ''; const events: ChatEvent[] = []
    const upd = (fn: (m: DisplayMessage) => Partial<DisplayMessage>) => setMessages(p => p.map(m => m.id === streamId.current ? { ...m, ...fn(m) } : m))
    chatStream({ message: text, sessionId: s },
      (e) => {
        if (e.type === 'ANSWER') { content += e.data; upd(() => ({ content })); scroll() }
        else if (['THINK', 'TOOL_CALL', 'TOOL_RESULT'].includes(e.type)) { events.push({ type: e.type as ChatEvent['type'], data: e.data, toolName: e.toolName }); upd(() => ({ events: [...events] })) }
      },
      () => { setSending(false); upd(() => ({ content: content || '请求失败', isStreaming: false })) },
      () => { setSending(false); upd(() => ({ isStreaming: false })); loadSessions() },
      abort.signal)
  }, [input, sending, sid, loadSessions, scroll])

  useEffect(() => { loadSessions() }, [loadSessions])

  const fmtTime = (ts: number) => new Date(ts).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })
  const fmtTitle = (t: string) => { if (!t) return ''; const d = new Date(t); return Date.now() - d.getTime() < 86400000 ? d.toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' }) : `${d.getMonth() + 1}/${d.getDate()}` }

  return (
    <Layout style={{ height: '100vh', background: '#f2f3f5' }}>
      <Sider width={280} style={{ background: '#fafafa', borderRight: '1px solid #f0f0f2', display: 'flex', flexDirection: 'column' }}>
        {/** 图标 */}
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '4px 8px' }}>
          {sessions.length === 0
            ...
            :
            sessions.map(s =>
              <div key={s.sessionId} onClick={() => select(s.sessionId)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', borderRadius: 8, cursor: 'pointer', marginBottom: 2, background: sid === s.sessionId ? '#e8f0fe' : 'transparent' }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <Text ellipsis style={{ fontSize: 13, fontWeight: sid === s.sessionId ? 600 : 400, display: 'block' }}>{s.title || '新对话'}</Text>
                  <Text type="secondary" style={{ fontSize: 11 }}>{s.messageCount} 条 · {fmtTitle(s.lastActiveTime)}</Text>
                </div>
                <Button size="mini" type="text" icon={<IconDelete />} onClick={(e: any) => del(s.sessionId, e)} />
              </div>
            )
          }
        </div>
      ...
      <Content style={{ display: 'flex', flexDirection: 'column', background: '#f2f3f5' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '12px 24px', flexShrink: 0 }}>
          <Space>
            <Avatar size={32} style={{ background: 'linear-gradient(135deg, #4d6bfe, #8b5cf6)' }}>{user?.username?.[0]?.toUpperCase() || 'U'}</Avatar>
            <div>
            ...
            : <>
              ...
              }
              {
                messages.map((msg, idx) => {
                  const u = msg.role === 'user', last = idx === messages.length - 1
                  return <div key={msg.id} className={last ? 'msg-in' : ''} style={{ display: 'flex', gap: 12, padding: '16px 24px', maxWidth: 820, margin: '0 auto', width: '100%', boxSizing: 'border-box', flexDirection: u ? 'row-reverse' : 'row' }}>

                    {/* 头像 */}
                    <Avatar size={34} style={{ flexShrink: 0, marginTop: 2, background: u ? 'linear-gradient(135deg, #4d6bfe, #3b5de7)' : 'linear-gradient(135deg, #8b5cf6, #6d28d9)' }}>{u ? <IconUser /> : <IconRobot />}</Avatar>
                    ...
                    <div style={{ maxWidth: 'calc(100% - 80px)', display: 'flex', flexDirection: 'column', alignItems: u ? 'flex-end' : 'flex-start' }}>
                      ...
                      {/* 对话内容 */}
                      {
                        msg.content
                          ...
                          :
                          msg.isStreaming ? <div style={{ padding: '12px 24px', borderRadius: '14px 14px 14px 4px', background: '#fff', border: '1px solid #f0f0f2' }}><Dots /></div> : null
                      }
        ...
        <div style={{ padding: '0 24px 20px', flexShrink: 0 }}>
          <div style={{ maxWidth: 820, margin: '0 auto', display: 'flex', gap: 8, alignItems: 'flex-end', background: '#fff', borderRadius: 16, padding: '8px 8px 8px 18px', border: `2px solid ${input.trim() ? '#4d6bfe' : '#e5e6eb'}` }}>
            <textarea ref={inputRef} placeholder="输入你的问题，Enter 发送，Shift+Enter 换行" value={input} onChange={e => setInput(e.target.value)} rows={1}
            ...
            {sending ? <Button icon={<IconStop />} shape="circle" onClick={stop} style={{ flexShrink: 0, color: '#f53f3f' }} />
              : <Button type="primary" icon={<IconSend />} shape="circle" onClick={send} style={{ flexShrink: 0 }} disabled={!input.trim()} />}
          </div>
          <Text type="secondary" style={{ display: 'block', textAlign: 'center', fontSize: 11, marginTop: 10 }}>AI 回复仅供参考，请验证重要信息的准确性</Text>
        </div>
      </Content>
    </Layout>
  )
